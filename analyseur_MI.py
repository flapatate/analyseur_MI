# -*- coding: utf-8 -*-
"""
Created on Mon Jul 27 19:21:38 2015
...
dossier_racine = 'C:\\Users\\Flap\\Dropbox\\academique\\these\\'

#fichier_corpus = '.\\SC_10_exemple.txt'
fichier_corpus = '.\\sous_corpus_21.txt'

fichier_unites = '.\\liste_signifiants_COMPLET.txt'

fichier_classifieur = '.\\classifieur_SVM.pkl'

fichier_etiqueteur = '.\\etiqueteur_ngrammes.pkl'
   
fichier_vectoriseur = '.\\vectoriseur.pkl'
...
def main():
    ...
    sorted_freq_conv = sorted(freq_voc_conversation.items(), key=operator.itemgetter(1))
    ...
    for enonciateur in freq_voc_enonciateur:
        ...
        sort_list = sorted(freq_voc_enonciateur[enonciateur].items(), key=operator.itemgetter(1))
 ...
 #   remplacer les _fin_citation    
  ...
  #  remplacer les flèches par _imm et _idd
   ...
   # remplacer les / pas _im et les \ par _id
    ...
    line = re.sub ('\t', ' ', line)
    line = re.sub ("\u00A0", ' ', line)            #fuckin espaces insecables
...
#START#
"""
corpus_root = 'C:\\Users\\Flap\\Documents\\SpiderOak Hive\\mraitagger\\corpus_cfpq\\'
corpus_txts = 'C:\\Users\\Flap\\Documents\\SpiderOak Hive\\mraitagger\\corpus_cfpq\\txts\\'

fichier_total = open ('.\\corpus_total.txt', "w", encoding='utf-8')

#text_name = '1_sous_corpus_1_ao_t2012_.txt'
#text_name = 'corpus30.txt'
#text_name = 'souscorpus5.txt'

files = os.listdir(corpus_txts)

for file in files:    

    fichier_propre = open ('.\\corpus_cfpq\\propres\\propre_' + file, "w", encoding='utf-8')
    fichier_tagge = open ('.\\corpus_cfpq\\tagges_automatique\\tagge_' + file, "w", encoding='utf-8')
    
    #text = PlaintextCorpusReader(corpus_root, text_name)
    text = PlaintextCorpusReader(corpus_txts, file)
    
    text = text.raw()
    
    #enleve lignes pas rapport
    text = re.sub ("^SOUS-CORPUS.*\r", '', text)
    text = re.sub ("\nSOUS-CORPUS.*\r", '', text)
    text = re.sub ("\n\d{0,3}\r", '', text)
    
#SOUS-CORPUS 5 : segment 1. (Durée 10 minutes)    
    
    #resplit comme il faut quand le pdf est brise 
    text = re.sub("\r\n", " ", text)
    text = re.sub ("(?P<enonciateur>\w{1,3}) : ", "\r\n\g<enonciateur> : ", text)
    ...
    for line in text:
        ...
        if re.match("^ \w", line):  #enleve lignes vides
            fichier_propre.write(line)
    
            line = taggeur_manuel(line)
            
            line = re.sub("^ ", "", line) #enleve espace au debut ligne pour tag   
            
            fichier_tagge.write(line)
            fichier_total.write(line)
        else:
            print ("no match")
